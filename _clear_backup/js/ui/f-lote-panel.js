...
(function() {
  ...
  function formatToHashtags(str) {
    ...
    if (trimmed.includes('#')) {
      const linesOrChunks = trimmed.split(/[,;\n]+/);
      for (let chunk of linesOrChunks) {
        chunk = chunk.trim();
        if (!chunk) continue;

        const hashMatches = chunk.match(/#[^\s#,;.]+/g);
        if (hashMatches && hashMatches.length > 0) {
          ...
          const remaining = chunk.replace(/#[^\s#,;.]+/g, '').trim();
          if (remaining) {
            const formatted = '#' + remaining.split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('');
            if (!items.includes(formatted)) items.push(formatted);
          }
        } else {
    ...
    if (isSpectator) {
      ...
      if (tagsContainer) {
        ...
        if (line.caracteristicas && line.caracteristicas.trim()) {
          const formattedStr = formatToHashtags(line.caracteristicas);
          const tags = formattedStr.split(/\s+/).filter(Boolean);
          if (tags.length) {
            tags.forEach(tag => {
              const cleanTagText = tag.replace(/^#+/, '');
              const span = document.createElement('span');
              span.className = 'kpk-spec-tag';
              span.innerHTML = `<span class="hashtag-symbol">#</span>${cleanTagText}`;
